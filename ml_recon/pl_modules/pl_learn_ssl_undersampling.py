...
class LearnedSSLLightning(plReconModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            mask_lambda = mask_lambda[0].cpu().numpy()
            mask_inverse = mask_inverse[0].cpu().numpy()
            initial_mask = initial_mask[0].cpu().numpy()
            ...
            wandb_logger.log_image('val/omega_lambda', np.split(mask_lambda.cpu().detach().numpy(), mask_lambda.shape[0], 0))
            wandb_logger.log_image('val/omega_(1-lambda)', np.split(mask_inverse.cpu().detach().numpy(), mask_lambda.shape[0], 0))
            wandb_logger.log_image('val/initial_mask', np.split(initial_mask.cpu().detach().numpy(), initial_mask.shape[0], 0))
